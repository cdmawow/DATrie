...
public:
    ...
    int LongestMatchSearch(const char *key, size_t &match_len, size_t key_len = 0) const;

    // judge whether key exists in dictionary
    // return key's property value on exist otherwise return -1
    int ExactMatchSearch(const char* key, size_t key_len = 0) const;
